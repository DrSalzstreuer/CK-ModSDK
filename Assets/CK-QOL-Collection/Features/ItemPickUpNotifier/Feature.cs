...
namespace CK_QOL_Collection.Features.ItemPickUpNotifier
{
    ...
    internal class Feature : FeatureBase
    {
        ...
        /// <inheritdoc/>
        /// <remarks>
        ///     Checks whether the feature can execute by validating that no inventory UI is open, no cutscenes are playing, and the scene handler is ready.
        /// </remarks>
        public override bool CanExecute() => base.CanExecute() && !Manager.ui.isPlayerInventoryShowing && !Manager.sceneHandler.cutsceneIsPlaying && Manager.sceneHandler.isSceneHandlerReady;
        ...
        /// <summary>
        ///     Updates the state of the Item Pick-Up Notifier feature, managing inventory changes and executing the notifier logic as needed.
        /// </summary>
        ...
        private void InitializePreviousInventoryState()
        {
            ...
            {
                _previousInventoryObjects.Add(default); // Fill with default values.
            }
        }
        ...
        private static void NotifyPlayerOfNewItem(ContainedObjectsBuffer containedObjectData)
        {
            string[] formatFields =
            {
                PlayerController.GetObjectName(containedObjectData, true).text
            };

            var rarity = PugDatabase.GetObjectInfo(containedObjectData.objectID).rarity;

            // Display the notification to the player via the chat window.
            Manager.ui.chatWindow.AddInfoText(formatFields, rarity, ChatWindow.MessageTextType.GainedItem);
        }
        ...
        /// </summary>
        /// <remarks>
        ///     This method ensures that all relevant inventory handlers (e.g., crafting and output inventories) are checked for new items.
